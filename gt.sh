...
version="0.2 (kether edition)"
function gittools() {
    while getopts ac:d:h"-help"psu:U: flag
    do
        case "${flag}" in
            a) add="git add -Av" >&2 ;;
            c) commit+="git commit -m '${OPTARG}'" >&2 ;;
            d) cherrypick="git cherry-pick '${OPTARG}'" >&2 ;;
            p) push="git push" >&2 ;;
            s) sync="git pull origin; git fetch sir" >&2 ;; # TODO: function
            u) H='HEAD~' update >&2 ;;
            U) update >&2 ;;
            h|-help|*) manual >&2 ;;
        esac
...
function manual() {
    ...
    echo    "  -d COMMITS    Dump (cherry-pick) commits from a remote repository"
